...
public class WolfSchedulerTest {
	...
	/**
	 * Test adding courses and events with conflicts to the schedule.
	 */
	@Test
	public void testAddCoursesAndEventsWithConflict() {
	    WolfScheduler ws = new WolfScheduler(validTestFile);

	    // Add a course to the schedule
	    assertTrue(ws.addCourseToSchedule("CSC 217", "202"));

	    // Try to add a course that conflicts with the existing course
	    try {
	    ...
	    // Try to add an event that conflicts with the existing course
	    try {
	        ws.addEventToSchedule("Meeting", "M", 1000, 1445, "Important meeting");
	        fail("Expected ConflictException");
	    } catch (IllegalArgumentException e) {
